#!/usr/bin/env python3
"""
EMO Options Bot Web Dashboard
Provides a web interface with ML outlook integration.
"""
...
# --- ML outlook support ---
ROOT = Path(__file__).resolve().parents[2]  # src/web/ -> project root
ML_JSON = ROOT / "data" / "ml_outlook.json"

def _read_ml_outlook():
...
def _render_ml_card():
    data = _read_ml_outlook()
    if not data or not data.get("symbols"):
        return '<div class="card"><h2>ML Outlook</h2><div style="color:#777">No ML outlook yet.</div></div>'
    rows = []
    for s in data["symbols"]:
        trend_color = "#4CAF50" if s.get('trend') == 'up' else "#f44336" if s.get('trend') == 'down' else "#FF9800"
        ...
        rows.append(
            ...
            f"<td style='color:{trend_color};font-weight:bold'>{s.get('trend','').upper()}</td>"
            f"<td style='color:{confidence_color};font-weight:bold'>{confidence:.3f}</td>"
            f"<td>{(s.get('expected_return') or 0):.4f}</td>"
            f"<td>{s.get('notes','')}</td>"
            f"</tr>"
        )
    html = (
...
def _get_database_status():
    ...
    db_path = ROOT / "data" / "describer.db"
...
def serve_dashboard(host='localhost', port=8083):
    ...
    print(f"💾 Database: {ROOT}/data/describer.db")
    print(f"📈 ML Data: {ROOT}/data/ml_outlook.json")
    print()
...
def start_dashboard(host: str = "localhost", port: int = 8083):
    """Start the dashboard server with custom host/port"""
